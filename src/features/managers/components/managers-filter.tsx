        ...
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-72 rounded-lg border border-border bg-card p-4 shadow-lg">
            <div className="mb-4 flex items-center justify-between">
              ...
              {activeCount > 0 && (
                <button type="button" onClick={onClearFilters} className="flex items-center gap-1 text-[10px] font-medium text-primary hover:underline">
                  <X className="size-3" />
                  Clear all
                </button>
              )}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {group.options.map((option) => (
                        <button
                          key={option.value}
                          type="button"
                          onClick={() => onFilterChange(group.key, activeFilters[group.key] === option.value ? "" : option.value)}
                          className={cn(
            ...
            <div className="mt-4 flex items-center justify-end gap-2 border-t border-border pt-3">
              ...
              <Button size="sm" onClick={() => { onApplyFilters(); setIsOpen(false); }}>Apply Filters</Button>
            </div>
          </div>
        </>
      )}
